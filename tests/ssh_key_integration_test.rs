...
use common::TestEnv;
use gt::io::ssh_key::{
    generate_key, read_public_key, set_key_permissions, verify_key, KeyGenOptions, KeyType,
};
use std::path::Path;
...
fn test_key_already_exists_without_force() {
    ...
    assert!(
        result.is_err(),
        "Should fail when key exists without force flag"
    );
...
fn test_key_overwrite_with_force() {
    ...
    assert!(
        result.is_ok(),
        "Should succeed with force flag after manual deletion"
    );
